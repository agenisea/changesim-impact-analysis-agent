export type Scope = 'single' | 'team' | 'department' | 'organization' | 'national' | 'global';
...
export function mapRiskLevel(
  ...
  if (s >= 4 && (sev >= 2 || h >= 2)) return { level: 'critical' }; // national/global + major/significant
  if (t === 3 && sev >= 2) return { level: 'critical' }; // time=critical + major+

  // ---- HARD CAPS ----
  // 1) Single-person guardrail: cannot exceed medium unless critical already matched
  if (s === 0 && h < 3) return { level: 'medium' };

  // 2) Org-cap (NEW): organization scope + limited/no human impact + not urgent + ≤ major => medium
  if (s === 3 && h <= 1 && t <= 1 && sev <= 2) return { level: 'medium', orgCapTriggered: true };
  ...
  if (s >= 4) return { level: 'high' }; // national/global (non-critical)
  ...
  if (s <= 1 && sev <= 1 && h === 0) return { level: 'low' };
  return { level: 'medium' };
}